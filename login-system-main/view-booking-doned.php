...
if (isset($_GET['id'])) {
    ...
    // Now fetch details from the 'test_result' table based on the 'booking_id'
    $testResultQuery = $connect->prepare("SELECT * FROM test_result WHERE booking_id = ?");
    if (!$testResultQuery) {
        die("Error in test result query: " . $connect->error);
    }

    $testResultQuery->bind_param("i", $bookingId);
    $testResultQuery->execute();

    $testResultResult = $testResultQuery->get_result();

    if (!$testResultResult) {
        die("Error fetching test result details: " . $testResultQuery->error);
    }

    $testResultDetails = $testResultResult->fetch_assoc();

    // Close the test result query statement
    $testResultQuery->close();

    // Now you have $testResultDetails with information from the 'test_result' table related to the 'booking_id'
} else {
...
<body style=" background-color: #f8f9fa;">
  <?php include 'partials/nav.php'?>
    <div class="container mt-3">  
                    ...
                    <div class="card shadow mb-4" style="height: 980px;">
   ...
   <td class="text-end" style="vertical-align: middle;">Application Date<input id="appDate" class="form-control" style="max-width: 200px; display: inline-block; margin-left: 8px;" value="<?php echo $bookingDetails['app_date']; ?>" type="text" readonly disabled></td>
    ...
    <div class="card-header d-flex justify-content-between align-items-center" style="background: var(--bs-success-text-emphasis);">
        <h6 class="text-primary fw-bold m-0" style="background: Transparent;"><span style="color: rgb(244, 248, 244);">DETAILS</span></h6>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table">
...
<tr>
<td class="text-center" style="font-size: 23px; color: <?php echo ($testResultDetails['testing_status'] == 1) ? 'green' : 'inherit'; ?>;">
    <?php echo ($testResultDetails['testing_status'] == 1) ? 'PASSED' : strtoupper($testResultDetails['testing_status']); ?>
</td>


</tr>
<tr>
    <th><span style="font-weight: normal !important;">RECORD</span></th>
</tr>
<tr>
<td class="text-center" style="font-size: 23px; color: <?php echo ($testResultDetails['record_status'] == 1) ? 'green' : 'inherit'; ?>;">
    <?php echo ($testResultDetails['record_status'] == 1) ? 'CERTIFIED' : strtoupper($testResultDetails['testing_status']); ?>
</td>
        ...
        <table class="table">
            ...
            <thead>
                ...
                <tr>
                    ...
                    <th><input id="hcInput" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['HC']; ?>" readonly disabled></th>
                    <th><span style="font-weight: normal !important;">CO</span></th>
                    <th> <input id="coInput" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['CO']; ?>"  readonly disabled></th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td>CO2</td>
                    <td><input id="co2Input" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['CO2']; ?>" readonly disabled></td>
                    <td>O2</td>
                    <td><input id="o2Input" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['O2']; ?>" readonly disabled></td>
                </tr>
                <tr>
                    <td>N</td>
                    <td><input id="nInput" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['N']; ?>" readonly disabled></td>
                    <td>RPM</td>
                    <td><input id="rpmInput" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['HC']; ?>" readonly disabled></td>
                </tr>

<tr> <td>K-AVE</td>
    <td colspan="3" style="text-align: center;">
    <input id="kAveInput" type="text" class="form-control" style="width: 100%;" value="<?php echo $testResultDetails['K_AVE']; ?>" readonly disabled>
    </td>
</tr>     
...
<div class="modal fade" id="confirmGoBackModal" tabindex="-1" role="dialog" aria-labelledby="confirmGoBackModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure you want to go back to schedule status?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
