...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($receiptImage['tmp_name'], $targetFile)) {
        ...
        $updateQuery = $connect->prepare("UPDATE car_emission SET  receipt2 = ?, payAmount2 = ?, paymentMethod2 = ?, reference2 = ?, paymentlock2 = ?, return_switch_2 = ? WHERE id = ?");
        if (!$updateQuery) {
            die("Error in update query: " . $connect->error);
        }

        $updateQuery->bind_param("ssssssi", $uniqueFilename, $payAmount1, $paymentMethod, $referenceNumber, $paymentLock1, $returnSwitch1, $bookingId);
        $updateQuery->execute();
